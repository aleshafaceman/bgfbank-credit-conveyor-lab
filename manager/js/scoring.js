...
function scoringStepsForApp(app, mode) {
    ...
    if (steps[2]) {
        steps[2].detail_ok = ndflOk
            ...
            : '2-НДФЛ в цифровом профиле нет — нужна справка о доходе.';
    }
    if (steps[4]) {
...
function showSResult(outcome) {
    ...
    if (sIssueLog.length > 0) {
        h += '<div style="margin-bottom:14px;"><div style="font-weight:700;font-size:12px;color:#0B4697;margin-bottom:6px;">📋 Обнаруженные проблемы:</div>';
    ...
    if (sMode === 'prescore') {
        if (outcome === 'approved') {
            ...
            h += '<div class="s-result approved"><div class="r-icon">✅</div><div class="r-title" style="color:#065f46;">Клиент предварительно подходит</div><div class="r-desc">Паспорт и БКИ хватило для рисковой модели. Это ещё не финальное одобрение — условия могут измениться на полном скоринге.</div><div class="r-params"><div class="r-param"><div class="r-label">Лимит</div><div class="r-value" style="color:#0B4697;">' + lim + '</div></div><div class="r-param"><div class="r-label">Предв. ставка</div><div class="r-value" style="color:#b45309;">' + rt + '</div></div><div class="r-param"><div class="r-label">Срок</div><div class="r-value">' + tm + '</div></div><div class="r-param"><div class="r-label">Предв. платёж</div><div class="r-value">' + pay + '</div></div></div><div class="s-btn-row"><button class="s-btn s-btn-outline" onclick="closeManagerScoring()"><i class="fas fa-times"></i> Закрыть</button></div></div>';
        } else {
        ...
        h += '<div class="s-result approved"><div class="r-icon">✅</div><div class="r-title" style="color:#065f46;">Кредит одобрен</div><div class="r-desc">Полный скоринг по оригиналам завершён. Статус заявки обновлён.</div><div class="r-params"><div class="r-param"><div class="r-label">Лимит</div><div class="r-value" style="color:#0B4697;">' + lim + '</div></div><div class="r-param"><div class="r-label">Итоговая ставка</div><div class="r-value" style="color:#13A538;">' + rt + '</div></div><div class="r-param"><div class="r-label">Срок</div><div class="r-value">' + tm + '</div></div><div class="r-param"><div class="r-label">Итоговый платёж</div><div class="r-value">' + pay + '</div></div></div><div class="s-btn-row"><button class="s-btn s-btn-success" onclick="confirmManagerScoringDecision(\'approved\')"><i class="fas fa-check"></i> Готово</button><button class="s-btn s-btn-outline" onclick="closeManagerScoring()"><i class="fas fa-times"></i> Закрыть</button></div></div>';
    } else {
