...
function renderClientsTab() {
    ...
    clientList.forEach(function(c) {
        ...
        h += '<div style="width:40px;height:40px;border-radius:50%;background:#0B4697;color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;">' + initials + '</div>';
...
function openClientProfile(clientName) {
    ...
    document.querySelectorAll('.client-list-item').forEach(function(item) {
        ...
        if (item.getAttribute('data-client') === clientName) {
            ...
            item.style.border = '1px solid #0B4697';
        }
    });
    ...
    h += '<div class="m-client-avatar" style="width:56px;height:56px;border-radius:50%;background:#0B4697;color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:20px;">' + (client.name || '?').split(' ').map(function(w){return w[0];}).join('') + '</div>';
    ...
    h += '<div class="m-client-section" style="margin-bottom:24px;"><h4 style="font-size:15px;color:#0B4697;margin-bottom:12px;"><i class="fas fa-file-signature"></i> Заявки (' + apps.length + ')</h4>';
    apps.forEach(function(a) {
        ...
        h += '<div><div class="m-app-number" style="font-weight:700;font-size:13px;color:#0B4697;">№' + a.id + '</div><div class="m-app-meta" style="font-size:11px;color:#94a3b8;">' + (a.date || '') + ' · ' + (a.amount != null ? a.amount : 0).toLocaleString('ru-RU') + ' ₽</div></div>';
        h += '<span class="m-badge badge-' + (a.status || 'processing') + '">' + (a.statusLabel || a.status || '') + '</span>';
        h += '</div>';
    });
    h += '</div>';
    
    h += '<div class="m-client-section" style="margin-bottom:24px;"><h4 style="font-size:15px;color:#0B4697;margin-bottom:12px;"><i class="fas fa-home"></i> Недвижимость</h4>';
    if (!properties.length) {
        h += '<div style="color:#94a3b8;font-size:13px;">Нет объектов</div>';
    }
    properties.forEach(function(p) {
        h += '<div style="background:#f8fbff;padding:10px;border-radius:10px;margin-bottom:6px;"><div style="font-size:11px;color:#7e9bb6;">' + (p.type || 'Объект') + ' · ' + (p.area != null ? p.area : '—') + ' м²</div><div style="font-weight:600;font-size:12px;">' + (p.address || '—') + '</div>' + (p.valuation ? '<div style="color:#13A538;font-size:12px;">' + p.valuation.toLocaleString('ru-RU') + ' ₽</div>' : '') + '</div>';
    });
    h += '</div>';
    
    h += '<div class="m-client-section" style="margin-bottom:24px;"><h4 style="font-size:15px;color:#0B4697;margin-bottom:12px;"><i class="fas fa-file-alt"></i> Документы</h4>';
    allDocs.forEach(function(d) {
        h += '<div class="m-doc-item" style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:#f8fbff;border-radius:10px;margin-bottom:6px;font-size:13px;"><i class="fas ' + (d.uploaded ? 'fa-check-circle' : 'fa-times-circle') + '" style="color:' + (d.uploaded ? '#13A538' : '#dc2626') + ';"></i><span class="doc-name" style="flex:1;font-weight:600;">' + d.name + '</span><span class="doc-status" style="font-size:11px;padding:3px 8px;border-radius:6px;font-weight:600;background:' + (d.uploaded ? '#d1fae5' : '#fee2e2') + ';color:' + (d.uploaded ? '#065f46' : '#991b1b') + ';">' + (d.uploaded ? 'Загружен' : 'Отсутствует') + '</span></div>';
    });
    ...
    h += '<div class="m-client-section" style="margin-bottom:24px;"><h4 style="font-size:15px;color:#0B4697;margin-bottom:12px;"><i class="fas fa-history"></i> История</h4>';
