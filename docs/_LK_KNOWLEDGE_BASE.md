...
Контуры LAB, которые **не стыкованы ссылкой** (нет handoff из кабинета в АРМ):
...
3. **АРМ андеррайтера `underwriter/`** — очереди АНД и АПЗ после СБ; барьер «клиент одобрен» + «залог одобрен»; паспорт и КОД не здесь.
4. **АРМ продуктолога `productolog/`** — конфиг продуктов/шкал/матрицы для Solver, не Loginom и не очередь заявок.
