...
|--------|---------|--------------|
| `25BGFB00990001` | ЕСИА, счёт не открыт | Полная СОПД действует → сверка ЦПГ → ЦФТ → SMS-заявление → КОД (электронная + УКЭП) |
| `25BGFB00990002` | без ЕСИА, счёт не открыт | **Короткая СОПД** — шаблон полной на стол → печать+скан заявления → КОД бумага |
| `25BGFB00990003` | ЕСИА, счёт уже есть | **СОПД просрочено** — шаблон полной; заявление на счёт пропускаем |

1. Выбрать заявку. В блоке СОПД: первое касание (лид ELMA), форма/срок первого согласия, «Скачать и проверить», при необходимости «Шаблон полная для подписания» (дата лида в шаблон не идёт).
