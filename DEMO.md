...
|--------|---------------|------|--------------|
| `25BGFB00990001` | SMS, полная электронная, действует | да, счёт не открыт | Шаблон полной **скрыт**. Сверка паспорта с данными ЦПГ (без фото). SMS-заявление → КОД электронная + УКЭП |
| `25BGFB00990002` | партнёр, короткая | нет, счёт не открыт | «Подписана короткая форма партнёра…» + **шаблон полной**. Очная сверка с анкетой. Печать+скан заявления → КОД бумага |
| `25BGFB00990003` | менеджер, полная бумага, **просрочена** | да, счёт уже есть | Шаблон полной на стол; заявление на счёт пропускаем |

1. Выбрать заявку. В блоке СОПД: первое касание (лид ELMA), форма/версия/срок/канал, «Скачать и проверить»; «Шаблон полная для подписания» только если первое СОПД недостаточно (короткая, просрочена или чужая версия). Дата лида в шаблон не идёт.
2. Отметить идентификацию и телефон.
3. «Искать счёт в ЦФТ» — `FindRetailAccount`; если счёта нет — шесть автопроверок (все успех).
