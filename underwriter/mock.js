window.UNDERWRITER_MOCK = {
  /* Дежурные стола. */
  officers: {
    and: { name: "Лаптев С. В.", role: "АНД" },
    apz: { name: "Демидова О. К.", role: "АПЗ" }
  },

  /* Заседание кредитного комитета. Заседание собирает система по регламенту:
     состав — по признакам заявки, слот — из графика заседаний. Человек
     (председатель КК) подтверждает дату и состав и рассылает приглашения.
     ФИО участников вымышленные: настоящих сотрудников банка в макете нет. */
  kk: {
    levels: ["committee", "board", "council"],
    level_titles: { committee: "Комитет", board: "Правление", council: "Совет директоров" },
    /* Предложенные слоты: система ставит заседание в ближайшее окно графика. */
    slots: [
      { date: "12.06.2026", time: "15:00", format: "заочно, видеоконференция" },
      { date: "13.06.2026", time: "11:30", format: "очно, зал заседаний" }
    ],
    /* Время в кадре макета. У стола все даты вымышленные и статичные, поэтому
       приглашения и позиции подписаны кадровым временем, а не часами машины:
       иначе заседание 12.06 оказывалось бы раньше рассылки приглашений. */
    frame: { invitations_at: "10.06.2026 09:40", answered_at: "12.06.2026 15:05" },
    /* Обязательный участник — без его позиции решение не собрать (кворум).
       when: "commerce" — приходит только по заявке с коммерческой недвижимостью. */
    roster: {
      committee: [
        { id: "m_chair", who: "Родионов А. Г.", role: "Руководитель группы андеррайтинга",
          why: "председательствующий", required: true },
        { id: "m_underwriter", from_track: true, required: true },
        { id: "m_appraiser", who: "Величко И. П.", role: "Оценщик банка",
          why: "коммерческая недвижимость — оценка обязательна", required: true, when: "commerce" },
        { id: "m_sb", who: "Корнеев Д. С.", role: "Служба безопасности",
          why: "приглашается по решению председателя", required: false },
        { id: "m_lawyer", who: "Шарипов Т. Р.", role: "Юрист",
          why: "проверка правоустанавливающих документов", required: false },
        { id: "m_sales", who: "Мещерякова Н. В.", role: "Сотрудник продаж",
          why: "докладчик по заявке", required: true }
      ],
      board: [
        { id: "m_board_head", who: "Барсуков Ю. Л.", role: "Руководитель блока «Ипотека»",
          why: "председательствующий уровня Правления", required: true },
        { id: "m_board_sales", who: "Мещерякова Н. В.", role: "Сотрудник продаж",
          why: "докладчик по заявке", required: true },
        { id: "m_board_fin", who: "Асланова Л. М.", role: "Финансовый блок",
          why: "участник уровня Правления", required: true }
      ],
      /* На Правление и Совет директоров участники идут по должности, а не по
         фамилии: в макете у них ФИО нет — так и подписаны. */
      council: [
        { id: "m_council_chair", who: "Председатель Совета директоров", role: "Совет директоров",
          why: "вынесено Правлением", required: true },
        { id: "m_council_member", who: "Член Совета директоров", role: "Совет директоров",
          why: "вынесено Правлением", required: true }
      ]
    },
    /* Кто приходит от контура заявки: у АНД и АПЗ это разные андеррайтеры. */
    underwriter_by_track: {
      and: { who: "Ситникова Е. А.", role: "Андеррайтер заёмщика",
        why: "заявка проходит АНД в этом столе" },
      apz: { who: "Демидова О. К.", role: "Андеррайтер залога",
        why: "залоговая заявка, АПЗ в этом столе" }
    }
  },
